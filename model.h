...
public:
    ...
    //string tassign_for_author_suffix;//suffix for topic assignment to author file
    ...
    string tauthors_suffix;     // suffix for file containing author-per-topics
    ...
    int tauthors; // print out rop authors per each topic
    int withrawstrs;

    double ** p; // temp variable for sampling
    int ** z; // topic assignments for words, size M x doc.size()
    int ** x; // topic assignments for author, size M x doc.size()
    ...
    int ** na; // na[i][j]: number of author in documet i assigned to topic
    int * nasum;  // nasum[x]: total number of author assigned to topic j, size A
    ...
    double ** theta; // theta: document-topic distributions, size A x K
    ...
    //int save_model_tassign_for_author(string filename);
    ...
    void estimate();
    void sampling(int m, int n, int *top, int *auth);
    void compute_theta();
    void compute_phi();
