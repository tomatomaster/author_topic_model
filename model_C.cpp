...
int model::save_model_others(string filename) {
  ...
  void model::estimate() {
	...
	for (int n = 0; n < ptrndata->docs[m]->length; n++) {
      ...
      printf("logP = %f \n", logP());
      /*for debag print
      printf("nw[0][0] = %d\t",nw[0][0]);
      printf("nwsum[0] = %d\t",nwsum[0]);
      printf("na[0][0] = %d\t",na[0][0]);
      printf("nasum[0] = %d\t",nasum[0]);
      printf("p[0][0]  = %f\t",p[0][0]);
      */

      /*recording gibbs sampling
      for(int s=0; s<10; s++){
	for(int m=0; m<M; m++){
	    for (int n = 0; n < ptrndata->docs[m]->length; n++) {
	      // (z_i = z[m][n])
	      // sample from p(z_i|z_-i, w)
	      sampling(m, n, &recording_z[s], &recording_x[s]);
	    }
	}
	dataset[s] = ptrndata
      }*/
  ...
  //int a = ptrndata->docs[m]->authors[n];//check文章に割り当てられた著者
	...
	//printf("p[%d][%d]=%f nw[%d][%d]=%d nwsum[%d]=%d na[%d][%d]=%d nasum[%d]=%d \n", a, k, p[a][k], w, k, nw[w][k], k, nwsum[k], a, k, na[a][k], a, nasum[a]);
      }
    }
